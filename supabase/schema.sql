...
CREATE POLICY "참가자만 채팅방 조회" ON chat_rooms
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM chat_participants
...
CREATE POLICY "로그인 사용자 채팅방 생성" ON chat_rooms
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- 채팅방 참가자
ALTER TABLE chat_participants ENABLE ROW LEVEL SECURITY;

CREATE POLICY "참가자 정보 조회" ON chat_participants
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "로그인 사용자 참가자 추가" ON chat_participants
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "본인 참가 정보 수정" ON chat_participants
